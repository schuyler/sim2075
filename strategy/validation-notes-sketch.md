...
3. **Routing schema.** Every complaint either attributes to a parameter address — a marginal, a loading, an Ω entry, a cascade magnitude, an impact row/durability type, a missing event or transmission step (catalog gap) — or is logged `unattributable`. The unattributable rate is a headline metric, not discarded noise.
