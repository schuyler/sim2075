...
Different causal types have different epistemologies. The factor-explained variance should reflect what factors *can* explain for each type.

**See [[methodology/reference/variance-allocation]] for targets, rationale, and computation method.**

Summary: Type 1 gets highest factor-explained variance (0.70–0.80), Type 3 gets lowest (0.40–0.50) because resolution depends on intractable small-N actor decisions.

### Implications for Factor Loadings

When specifying factor loadings:
1. Determine causal type
2. Loadings must satisfy: **(ΛΩΛᵀ)ᵢᵢ = target** for that type
3. Specify *relative* loadings first, then scale to achieve target

Type 3 events will have lower absolute loadings than Type 1 events—this correctly reflects that factors explain less of Type 3 occurrence.
